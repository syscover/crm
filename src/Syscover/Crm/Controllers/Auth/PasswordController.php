...
class PasswordController extends Controller
{
    ...
    */
     ...
     * @var string
     */
    protected $broker = 'crmPasswordBroker';
    ...
    {
        $this->validate($request, ['email_301' => 'required|email']);

        $broker = $this->getBroker();

        $response = Password::broker($broker)->sendResetLink($request->only('email_301'), function (Message $message) {
    ...
    public function postReset(Request $request)
    {
        $this->validate($request, [
            'token'         => 'required',
            'email_301'     => 'required|email',
            'password'      => 'required|confirmed|min:6',
        ]);

        $credentials = $request->only(
            'email_301', 'password', 'password_confirmation', 'token'
        );
    ...
    protected function getResetFailureResponse(Request $request, $response)
    {
        return redirect()->back()
            ->withInput($request->only('email_301'))
            ->withErrors(['email_301' => trans($response)]);
    }
